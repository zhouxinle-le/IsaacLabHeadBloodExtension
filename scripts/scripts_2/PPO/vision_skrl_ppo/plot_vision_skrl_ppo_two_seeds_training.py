...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Plot two-seed Vision Wrist skrl PPO training curves."
    )
    parser.add_argument("--runs", type=Path, nargs="+", default=list(DEFAULT_RUNS), help="skrl run directories.")
...
def _read_curve(run_dir: Path, run_index: int, spec: MetricSpec) -> RawCurve:
    try:
        ...
        raise ModuleNotFoundError(
            ...
            "plot_vision_skrl_ppo_two_seeds_training.py"
        ) from exc
    ...
    if tag is None:
        ...
        raise KeyError(f"Missing TensorBoard tag in {run_dir}. Expected one of {expected}. Available tags: {available}")
    if tag != spec.requested_tag:
        print(f"[WARN] {spec.requested_tag!r} not found in {run_dir.name}. Using skrl tag {tag!r}.")
    ...
    order = np.argsort(x)
    return RawCurve(run_index, tag, x[order], y[order])
...
def _write_csv(aggregates: list[AggregateCurve], output_path: Path) -> None:
    ...
    fieldnames = ["metric", "tag", "trainer_step", "mean", "std"]
...
def main() -> None:
    ...
    for spec in METRICS:
        ...
        print(
            f"[INFO] {spec.key}: tag={aggregate.tag}, "
    ...
    csv_path = output_dir / "vision_skrl_ppo_two_seed_curves.csv"
