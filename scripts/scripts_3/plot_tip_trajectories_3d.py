...
def parse_trajectory_name(path: Path) -> dict[str, int | str | None]:
    ...
    match = re.match(
        r"^(?P<prefix>.+)_seed(?P<seed>\d+)_(?P<outcome>success|failure)(?P<outcome_index>\d+)_episode(?P<episode_id>\d+)$",
        stem,
    )
    if not match:
        return {"seed": None, "outcome": "unknown", "outcome_index": None, "episode_id": None}
    return {
...
def _select_one_trajectory_per_seed(trajectories: list[Trajectory]) -> list[Trajectory]:
    selected: list[Trajectory] = []
    for seed in (0, 1):
        candidates = [traj for traj in trajectories if traj.seed == seed]
        candidates.sort(
            key=lambda item: (
                item.outcome_index or 9999,
                item.episode_id or 9999,
                item.path.name,
            )
        )
        if candidates:
    ...
    if selected:
        return selected
    return sorted(trajectories, key=lambda item: (item.outcome_index or 9999, item.episode_id or 9999, item.path.name))[:2]
...
def _plot_outcome_figure(
    ...
    selected = _select_one_trajectory_per_seed(outcome_trajectories)
    if not selected:
