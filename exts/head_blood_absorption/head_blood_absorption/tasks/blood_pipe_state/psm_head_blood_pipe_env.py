...
@configclass
class PsmBloodPipeAbsorptionEnvCfg(DirectRLEnvCfg):
    episode_length_s = 10
    decimation = 2
    action_space = 3
    state_space = 0
...
class PsmBloodPipeAbsorptionEnv(DirectRLEnv):
    """低维观测的 PSM 粒子吸取环境."""

    cfg: PsmBloodPipeAbsorptionEnvCfg

    def __init__(self, cfg: PsmBloodPipeAbsorptionEnvCfg, render_mode: str | None = None, **kwargs):
    ...
    def _invert_quat_torch(quat_wxyz: torch.Tensor) -> torch.Tensor:
        quat = PsmBloodPipeAbsorptionEnv._normalize_quat_torch(quat_wxyz)
        return torch.cat((quat[..., :1], -quat[..., 1:]), dim=-1)

    @staticmethod
    def _multiply_quat_torch(lhs_wxyz: torch.Tensor, rhs_wxyz: torch.Tensor) -> torch.Tensor:
        lhs = PsmBloodPipeAbsorptionEnv._normalize_quat_torch(lhs_wxyz)
        rhs = PsmBloodPipeAbsorptionEnv._normalize_quat_torch(rhs_wxyz)
        ...
        return PsmBloodPipeAbsorptionEnv._normalize_quat_torch(result)
    ...
    def _get_dones(self) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        terminated, truncated, next_counter, next_pipe_counter, flags = self._compute_termination_flags(psm_contact_force)
