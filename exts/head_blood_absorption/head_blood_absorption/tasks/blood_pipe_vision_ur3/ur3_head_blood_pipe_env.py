...
from .vision_observation import BloodPipeVisionObservationManager


@configclass
class Ur3BloodPipeVisionEnvCfg(DirectRLEnvCfg):
    episode_length_s = 10
    decimation = 2
    action_space = 3
    state_space = 0
    ...
    pipe_camera: TiledCameraCfg = TiledCameraCfg(
        prim_path="/World/envs/env_.*/HeadPipe/head_pipe/pipe_Link/PipeCamera",
        offset=TiledCameraCfg.OffsetCfg(
            pos=(0.0, 0.0, 0.0847066222752),
...
class Ur3BloodPipeVisionEnv(DirectRLEnv):
    """腕部视觉 + 非特权本体观测的 UR3 粒子吸取环境."""

    cfg: Ur3BloodPipeVisionEnvCfg

    def __init__(self, cfg: Ur3BloodPipeVisionEnvCfg, render_mode: str | None = None, **kwargs):
        self._camera: TiledCamera | None = None
        self._vision_observation: BloodPipeVisionObservationManager | None = None
    ...
    def _init_robot_control_state(self) -> None:
        ...
        self._ik_joint_lower_limits = self._ur3.data.soft_joint_pos_limits[0, self._ik_joint_ids, 0].to(device=self.device)
        self._ik_joint_upper_limits = self._ur3.data.soft_joint_pos_limits[0, self._ik_joint_ids, 1].to(device=self.device)
    ...
    def _init_observation_cache(self) -> None:
        self._vision_observation = BloodPipeVisionObservationManager(
            cfg=self.cfg,
            num_envs=self.num_envs,
            device=self.device,
        )
        self._vision_observation.bind_runtime(camera=self._camera)
    ...
    def _build_proprio_observation(
        self,
    ...
    def _update_vision_observation(self) -> None:
        ...
        proprio_obs = self._build_proprio_observation(tip_pos_w, ur3_contact_force)
        self._vision_observation.update(proprio_obs)
    ...
    def _get_observations(self) -> dict:
        if self._observation_pending:
            self._update_vision_observation()

            self._step_count += 1
            self._maybe_save_blood_template()
            self._observation_pending = False

        return {"policy": self._vision_observation.get_observations()}
